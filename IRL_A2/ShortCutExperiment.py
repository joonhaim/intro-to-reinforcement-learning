...
#
# This script will run the experiments described in IRL_A2.pdf:
#   1. Q-Learning
#   2. SARSA
...
#   5. n-step SARSA
#
# It will produce plots of averaged returns vs. episode for various
# hyperparameter settings, and also show (in the terminal) the
# greedy policy learned by the agents for single long-run experiments.
###############################################################
...
################################################################
# 1. Adding Progress Prints in Agent "train" Methods
################################################################
#
# The easiest way to add progress printing is to insert a quick
# `if episode % K == 0: print(...)` inside each agent's `train()`.
#
# For example, in QLearningAgent.train(), you can do:
#
#     for episode in range(n_episodes):
#         if episode % 1000 == 0:
#             print(f"[Q-Learning] Episode {episode}/{n_episodes}")
#         ...
#
# Repeat similarly for SARSAAgent, etc., or add a `verbose` parameter
# if you only want to see progress sometimes.
#
# Alternatively, you can add a progress bar using `tqdm`, but simple
...
# Here, if you want a more general approach, you can modify `run_experiment`
# to do the printing. But typically it's easiest to modify the agent's own train().
...
def experiment_qlearning():
    ...
    plot_curves(
        ...
        "Q-Learning: 100 reps, 1k episodes",
...
def experiment_expectedsarsa():
    ...
    plot_curves(
        ...
        "Expected SARSA: 100 reps, 1k episodes",
...
def experiment_nstepsarsa():
    ...
    plot_curves(
        nsarsa_curves,
        nsarsa_labels,
        "n-step SARSA: 100 reps, 1k episodes",
...
if __name__ == "__main__":
    ...
    print("\nAll requested experiments completed. Figures have been saved.")
    print("You can now include these plots in your report and discuss the results.\n")
